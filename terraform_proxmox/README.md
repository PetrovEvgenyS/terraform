...
Провайдер Telmate не входит в официальный реестр HashiCorp, поэтому ставится вручную в локальный каталог плагинов.
...
Шаблон — это ВМ с ID 9000, превращённая в template. Terraform будет клонировать её (full clone) и настраивать через cloud-init.
...
В `providers.tf` заданы URL API и токен. **Рекомендация:** не хранить секреты в репозитории. Используйте переменные окружения:

```hcl
# В providers.tf можно использовать переменные:
# pm_api_url          = var.pm_api_url
# pm_api_token_id     = var.pm_api_token_id
# pm_api_token_secret = var.pm_api_token_secret
```

И передавать их через `export PM_API_URL=...`, `TF_VAR_pm_api_token_secret=...` или через `terraform.tfvars` (добавьте `terraform.tfvars` в `.gitignore`).
...
### 3.3 Пароль и SSH-ключи (main.tf)

В ресурсе `proxmox_vm_qemu` заданы `cipassword` и `sshkeys`. Для продакшена лучше вынести их в переменные и не коммитить в репозиторий (например, через `tfvars` в `.gitignore`).
